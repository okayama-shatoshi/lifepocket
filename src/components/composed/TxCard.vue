<template>
	<div class="tx-card no-scrollbar" :class="{ verticalElement }">
		<div class="tx-content" :class="{ 'flex-row': !verticalElement, 'flex-column': verticalElement }">
          ...
          <div v-if="isValue">
            <Amount :ar="value" />
            <div class="secondary-text">Payment</div>
          </div>
          <div v-else-if="isData">
            <div>{{ context }}</div>
            <div class="secondary-text">{{ dataType || 'Data' }}</div>
          </div>
          <div v-else>
            <div>Transaction</div>
...
const dataInfo = computed(() => {
  const fileName = tags.value['file-name'] || tags.value['File-Name']
  const folderName = tags.value['folder-name'] || tags.value['Folder-Name']
  
  if (fileName) return fileName
  if (folderName) return folderName
  
  return tags.value['app'] || tags.value['application'] || tags.value['app-name'] || tags.value['application-name']
    ...
    || tags.value['user-agent']?.split('/').join(' ')
    || tags.value['type']
    || 'Data'
})
