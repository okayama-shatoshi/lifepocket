...
export const gatewayDefault = 'https://gwnode4.321.io'
export const bundlerDefault = 'https://gwnode4.321.io/bundler'
...
async function loadGatewaySettings () {
	...
	if (!ArweaveStore.gatewayURL && !state.value && InterfaceStore.online) {
		...
		const isReachable = isLocal || await testGateway(gatewayDefault).catch(async e => {
			...
			const fallbackReachable = await updateArweave('https://gwnode4.321.io').catch(() => false)
